...
int main() {
  ...
  while (1) {
    x1_barrel = 150 * cos(angle1 - delta_angle);
    y1_barrel = 150 * sin(angle1 - delta_angle);
    x2_barrel = 150 * cos(angle1 + delta_angle);
    y2_barrel = 150 * sin(angle1 + delta_angle);
    ...
    if (gfx_isKeyDown(SDLK_RIGHT)) {
      angle1 -= 1.0 * (M_PI / 180.0);
      angle2 -= 1.0 * (M_PI / 180.0);
    }
    ...
    if (gfx_pollkey()== SDLK_SPACE) {
      ...
      bullet1.fire_angle = angle1;
      bullet2.distance = 200.0; // 200 just for debuging
      bullet2.fire_angle = angle2;
      is_shooting = 1;
      printf("Space");
    }
    ...
    if (is_shooting == 1) {
      //gfx_filledCircle(gfx_screenWidth() / 2, // circle Just for debugging
      //                 gfx_screenHeight() / 2, 60, GREEN);
    ...
    if (bullet_count >= 80) {
      //gfx_filledCircle(gfx_screenWidth() / 2, // circle Just for debugging
      //                 gfx_screenHeight() / 2, 200, BLACK);

      draw_bullet(bullet2.x, bullet2.y);
      bullet2.distance += 10.0;
    }
    ...
    if (bullet1.to_enemy_distance <= 20 || bullet2.to_enemy_distance <= 20) {
      int scale = 1;
      is_shooting = 0;
      //bullet_count = 0;
